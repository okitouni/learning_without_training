...
from lwot.models import GEMConv2d, GEMLinear
...
def conv_bn_relu(c_in, c_out, kernel_size=(3, 3), padding=(1, 1), CONV2D = nn.Conv2d):
    return nn.Sequential(
        CONV2D(c_in, c_out, kernel_size=kernel_size, padding=padding, bias=False),
        GhostBatchNorm(c_out, num_splits=16),
        nn.CELU(alpha=0.3),
    )


def conv_pool_norm_act(c_in, c_out, CONV2D = nn.Conv2d):
    return nn.Sequential(
        CONV2D(c_in, c_out, kernel_size=(3, 3), padding=(1, 1), bias=False),
...
class ResNetBagOfTricks(nn.Module):
    def __init__(self, first_layer_weights, c_in, c_out, scale_out, masking=False):
        super().__init__()
        if masking:
          CONV2D = GEMConv2d
          LINEAR = GEMLinear
        ...
        # TODO given that these weights are not updated anyway, should we still use GemMiner?
        conv1 = nn.Conv2d(c_in, c, kernel_size=(3, 3), padding=(1, 1), bias=False)
        conv1.weight.data = first_layer_weights
        conv1.weight.requires_grad = False

        self.conv1 = conv1
        self.conv2 = conv_bn_relu(c, 64, kernel_size=(1, 1), padding=0, CONV2D=CONV2D)
        self.conv3 = conv_pool_norm_act(64, 128, CONV2D=CONV2D)
        self.conv4 = conv_bn_relu(128, 128, CONV2D=CONV2D)
        self.conv5 = conv_bn_relu(128, 128, CONV2D=CONV2D)
        self.conv6 = conv_pool_norm_act(128, 256, CONV2D=CONV2D)
        self.conv7 = conv_pool_norm_act(256, 512, CONV2D=CONV2D)
        self.conv8 = conv_bn_relu(512, 512, CONV2D=CONV2D)
        self.conv9 = conv_bn_relu(512, 512, CONV2D=CONV2D)
        self.pool10 = nn.MaxPool2d(kernel_size=4, stride=4)
        self.linear11 = LINEAR(512, c_out, bias=False)
...
Model = lambda *args, **kwargs: ResNetBagOfTricks(*args, **kwargs)
