...
def dot_product_matching(m1, m2, inplace=True):
  ...
  for l1, l2, l3 in zip(m1.children(), m2.children(), m3.children()):
    if not isinstance(l1, torch.nn.Linear): continue
...
def test():
  w1_A = torch.tensor([[1, 2], [3, 4]])
  w1_B = torch.tensor([[3, 4], [1, 2]])

  # greedy dot product matching recovers this ^
  
  w2_A = torch.tensor([[5, 6], [7, 8]])
  w2_B = torch.tensor([[6, 5], [8, 7]])
  ...
  print("pre-matching")
  print("A")
  print(m1[0].weight.data, m1[1].weight.data)
  print("prod", m1[0].weight.data @ m1[1].weight.data)
  print("B")
  print(m2[0].weight.data, m2[1].weight.data)
  print("prod", m2[0].weight.data @ m2[1].weight.data)
